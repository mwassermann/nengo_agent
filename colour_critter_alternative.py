...
with model:
    ...
    def rgb_to_pointer(x):
        ...
        if color_str == "WHITE":
            return vocab.parse(color_str).v * 0.2 
        else:
    ...
    # previous no longer used in this version
    ...
    def learn_sequence(t, x):
        ...
        if current_white_sim > 0.6 or next_white_sim > 0.6:
            return (current * next_color) * 2.0
        else:
            return (current * next_color) * 6.0
    ...
    # Connect learning result to memory buffer and buffer to cleanup
    nengo.Connection(learn_node, model.buffer.input, transform=2.5)
    nengo.Connection(model.buffer.output, model.cleanup.input, transform=2.5)
    
    # Prediction node - uses current and previous colors to predict next
    def make_prediction(t, x):
        current = x[:D]
        learned = x[D:2*D]  # This should be from cleanup memory
        
        white_vec = vocab.parse('WHITE').v
        current_white_sim = np.dot(current, white_vec)
        
        # Use current color to probe learned associations, increased gap between scaling
        if current_white_sim > 0.6:
            return current * learned * 2.0
        else:
            return current * learned * 8.0
    ...
    # Clean up and output prediction with slightly different connections
    ...
    nengo.Connection(model.memory.output, predict_node[:D], transform=1.5)
    nengo.Connection(model.cleanup.output, predict_node[D:], transform=1.5)
    nengo.Connection(predict_node, model.pred_cleanup.input, transform=3.0)
    nengo.Connection(model.pred_cleanup.output, model.prediction.input, transform=3.0)
